...
const abiao_filter = require('abiao_filter')
// sqlserver数据库连接配置
const dbConfig = {
  user: "sa",
  password: "admin@123",
...
async function filterSensitive(text) {
  let result = await abiao_filter.default.filter(text)
  if(result.text) {
    return result.text
  }
  return text
...
async function transCn(enTitle, postId){
  try {
    ...
    let res = await filterSensitive(transResult.data.tgt_text)
    return res || ""
  } catch (error) {
...
async function getCnBody(body) {
    ...
    let res = await filterSensitive(transResult)
		return res
	}
	return null
}
...
async function main() {
  ...
  let startId = Number(startAndEnd[0]) + 2*count    // 区间起点
  let endId = Number(startAndEnd[0]) +  3*count // 区间重点
